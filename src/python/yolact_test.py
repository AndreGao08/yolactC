...
def save_mask(masks,mask_path):
    ...
    #name=folder+'/'+name.replace('.png', '_person_mask')+'.png'
    name=mask_path+'/mask.png'
    #print("创建mask文件夹")
    cv2.imwrite(name, masks)
    print("返回一个numpy类型的图像")
    return masks
...
def prep_display(dets_out, img, h, w,mask_path, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        ...
        mask_person=save_mask(mask_person, mask_path)
        return mask_person
    '''
...
def evalimage(image,net:Yolact, path:str, save_path:str=None):
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    cleanup_and_exit()
'''
def evaluate(image,net:Yolact, dataset, train_mode=False):
...
def print_maps(all_maps):
    ...
    print()
'''
def yolact__init(path):
    print("[python] start,init yolact model!")
    parse_args()
    #print("[python] over")
    str_temp=SavePath.from_str(path)
    ...
    with torch.no_grad():
            ...
            net = net.cuda()
            dataset = None
            print("[python] init yolact success!")
'''
class yolact_Interface:
    #初始化
    def __init__(self, weight_path):
        print("[yolact_model] start init")
        self._weight_path=weight_path
        str_temp=SavePath.from_str(weight_path)
        str_temp=str_temp.model_name+'_config'
        set_cfg(str_temp)
        with torch.no_grad():
            cudnn.fastest=True
    ...
    def eval_image(self,image):
        ...
        with torch.no_grad():
            ...
            mask_path="/home/simulator/Documents/yolact_C++/src/python/output"
            mask_person = prep_display(preds,frame,None,None,mask_path,undo_transform=False)
        return mask_person,mask_person





def yolact__init__(weight_path):
    global model_interface
    model_interface=yolact_Interface(weight_path)
    return

def evaluate(image):
    print("[Yolact|evalue] start")
    return model_interface.eval_image(image)

#初始化函数为yolact__init__(path)
#预测函数为evalue(image)
if __name__ == '__main__':
    path="weights/yolact_base_54_800000.pth"
    yolact__init__(path)
    print("[openCV] read image")
    image = cv2.imread("input/test.jpg")
    print(image.shape)
    mask_person=evaluate(image)
    print(type(mask_person))
