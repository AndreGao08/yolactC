...
#include "yolact.h"
namespace yolact {
    //pyFilePath为python文件的路径
    //model为.pth模型文件的路径
    //PY_ENV_PKGS_PATH："/home/simulator/anaconda3/envs/yolact/lib/python3.7/site-packages"
    //EVAL_PY_FUNCTION_NAME:evaluate
    //INIT_PY_FUNCTION_NAME:yolact__init__
    yolact::yolact(const std::string &pyFilePath,
        ...
        //std::cout<<mstrPyMoudlePath<<std::endl;
        ...
        //std::cout<<mstrPyEnvPkgsPath<<std::endl;
        sstrCommand.str("");
        sstrCommand.clear();
        sstrCommand << "sys.path.append('";
        ...
        std::cout<<"[C++]import numpy success!"<<std::endl;

        mbIsPythonInitializedOK = true;

        // step 3 导入初始化模块
        std::cout<<"PyModuleName:"<<mstrPyMoudleName<<std::endl;
        ...
        std::cout<<"[C++]import module success!"<<std::endl;
        std::cout<<"[C++] Python init success!"<<std::endl;
        ...
        std::cout<<"[C++] Read PyFunction success!"<<std::endl;
        PyObject *pArgs = PyTuple_New(1);
        PyTuple_SetItem(pArgs, 0, Py_BuildValue("s", modelPath.c_str()));

        if (PyEval_CallObject(pFunc, pArgs) != nullptr) {
            std::cout << "[C++] Constructor return is not zero." << std::endl;
        }
        std::cout<<"[C++] Yolact init success!"<<std::endl;
...
// 从给定的字符串中解析得到访问文件的目录和名字
                           ...
                           )
    {
        ...
        std::cout<<"[C++]Yolact::evalImage Enter. "<<std::endl;
         ...
         */
        cv::resize(imputImageCache,imputImageCache,cv::Size(640,480));
                                      ...
                                      PyObject*& pPyArray)
    {
        ...
        // 生成
        mpb8ImgTmpArray=new unsigned char[w*h*c];
                                      ...
                                      PyObject*& pPyArray)
    {
        ...
        size_t iRows     = srcImage.rows,
                iCols     = srcImage.cols*srcImage.channels();
